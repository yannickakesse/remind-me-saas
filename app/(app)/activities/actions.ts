...
export async function createActivity(formData: FormData) {
  ...
  if (activityError) {
    throw new Error("Impossible de créer l'activité. Vérifiez les champs.");
  }
  ...
  if (parsed.schedule.schedules.length > 0) {
    ...
    if (schedulesError) {
      throw new Error("L'activité a été créée mais les horaires n'ont pas pu être enregistrés.");
    }
  }
  ...
  if (compensationError) {
    throw new Error("L'activité a été créée mais la rémunération n'a pas pu être enregistrée.");
  }
...
export async function updateActivity(activityId: string, formData: FormData) {
  ...
  if (activityError) {
    throw new Error("Impossible de modifier l'activité.");
  }

  // Horaires : on remplace l'ensemble des créneaux (plus simple et plus sûr
  // qu'un diff fin pour le MVP — un identifiant stable par créneau pourra
  // être introduit si l'édition ligne à ligne devient nécessaire).
