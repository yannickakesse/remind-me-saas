  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-ink-950">Centre de notifications</h1>
            {unreadCount > 0 && (
          ...
          <p className="text-sm text-ink-500 mt-1">
            Rappels intelligents d'activités, factures clients, échéances financières et tâches.
          </p>
        </div>
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="inline-flex items-center justify-center px-4 py-2 rounded-xl bg-canvas-raised border border-ink-200 text-xs font-semibold text-ink-700 hover:bg-ink-100 active:scale-95 transition-all disabled:opacity-50 shadow-xs"
          >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
        <div className="flex items-center gap-1.5 p-1 rounded-xl bg-ink-100/60 border border-ink-200/60 overflow-x-auto">
          {[
            ...
            { key: "task", label: "Tâches & Activités", count: initialNotifications.filter((n) => n.category === "task" || n.category === "activity").length },
          ].map((tab) => (
            <button
              key={tab.key}
              type="button"
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === tab.key
                  ? "bg-canvas-raised text-ink-950 font-bold shadow-xs"
        ...
        <div className="relative w-full sm:w-64">
          <input
            ...
            className="w-full px-3 py-1.5 text-xs rounded-xl bg-canvas-raised border border-ink-200 text-ink-950 placeholder:text-ink-400 focus:outline-none focus:ring-2 focus:ring-signal"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery("")}
              className="absolute right-2.5 top-1.5 text-xs text-ink-400 hover:text-ink-700"
            >
              ✕
            </button>
          )}
            ...
            return (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  ...
                  {/* Right: Actions */}
                  <div className="flex flex-wrap sm:flex-col items-end gap-1.5 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-ink-100">
                    ...
                    {!isResolved && isIncome && item.entity_id && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-positive text-white text-xs font-semibold hover:bg-positive/90 active:scale-95 transition-all disabled:opacity-50 shadow-xs"
                      >
                        ✓ Marquer comme reçu
                      </button>
                    )}
                    ...
                    {!isResolved && isExpense && item.entity_id && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-signal text-white text-xs font-semibold hover:bg-signal/90 active:scale-95 transition-all disabled:opacity-50 shadow-xs"
                      >
                        ✓ Marquer comme payé
                      </button>
                    )}
                    ...
                    <div className="flex items-center gap-1">
                      {isUnread && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg text-xs font-medium text-ink-600 hover:text-ink-950 hover:bg-ink-100 transition-colors"
                        >
                          Marquer lu
                        </button>
                      )}
                      ...
                      <button
                        ...
                        className="px-2 py-1 rounded-lg text-xs font-medium text-ink-400 hover:text-danger hover:bg-danger-soft/20 transition-colors"
